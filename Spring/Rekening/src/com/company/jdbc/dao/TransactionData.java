...
public class TransactionData {
    ...
    public List<Transaction> getTransaction(String code){
        ...
        try {
            ...
            String query = "SELECT description, date, amount  " +
                    "FROM tb_transaction INNER JOIN tb_transaction_type " +
                    "WHERE tb_transaction.account_number_debit = ? OR tb_transaction.account_number_kredit=? "+
         ...
         //   String query = "SELECT * FROM tb_transaction WHERE account_number_debit = ? OR account_number_kredit= ? ";
            ...
            while (resultSet.next()){
                ...
                tr1.setTransactionType(resultSet.getString("description"));
                tr1.setDate(resultSet.getTimestamp("date"));
                tr1.setAmount(resultSet.getDouble("amount"));
                list.add(tr1);
            }
        }catch (SQLException se){
            se.printStackTrace();
