...
public class WalletData {
    ...
    public Wallet getByWallet(int id){
        String newId = String.valueOf(id);
        try {
            String query = "SELECT * FROM tb_wallet_account WHERE wallet_id=?";
            statement = connect.prepareStatement(query);
    ...
    public List<Wallet> getWalleteReg(String code ){
        ...
        try {
            String query = "SELECT tb_walet.*, tb_wallet_account.* FROM tb_walet, tb_wallet_account "+
                    "WHERE tb_walet.customer_number=? AND tb_walet.id=tb_wallet_account.wallet_id";
            statement = connect.prepareStatement(query);
            statement.setString(1,code);
            resultSet = statement.executeQuery();
    ...
    public void deleteWallet(int id, String code){
        try {
            String query = "DELETE FROM tb_walet WHERE id=? AND customer_number=?";
            statement = connect.prepareStatement(query);
            statement.setInt(1,id);
