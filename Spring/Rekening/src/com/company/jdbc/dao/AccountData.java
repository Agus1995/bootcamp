...
public class AccountData {
    ...
    public void deleteAccount(String id, String code){
        String query = "DELETE FROM tb_account WHERE account_number=? AND customer_number=?";
        try {
    ...
    public Account getByAccCss(String id, String code){
        try{
            String query = "SELECT * FROM tb_account WHERE account_number=? AND customer_number=?";
            statement = connect.prepareStatement(query);
            statement.setString(1,id);
            statement.setString(2,code);
            resultSet = statement.executeQuery();
            ...
            while (resultSet.next()){
                ...
                account.setCustomer_number(resultSet.getString("customer_number"));
                return account;
            }
        }catch (SQLException se){
            se.printStackTrace();
